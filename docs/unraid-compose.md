# Unraid: the bundled stack (if you don't have a reverse proxy yet)
...
If you already followed the main [README](../README.md) and set up Cloudflare Tunnel or a reverse proxy, you don't need this page.

1. **Apps tab** (Community Applications) → search **"Compose Manager"** (by dcflachs) → **Install**.
2. A new **Compose Manager** tab appears (usually under **Settings**). Click **Add New Stack**, name it `multiplayer-markdown`.
...
4. This stack has its own `.env` editor (same GUI, no command line). Set:
...
5. Create the file the stack needs at `/mnt/user/appdata/multiplayer-markdown/Caddyfile` (each stack has a small file editor next to its compose box), with the exact contents of this repo's [`Caddyfile`](../Caddyfile).
6. Start the stack. Check the `sync-server` log for a "running" message, and the `caddy` log for "certificate obtained successfully".
7. If you're using a real domain (not `localhost`): forward ports 80 and 443 on your router to your Unraid box, and point your domain's DNS at your home internet's public IP. Without this, Caddy can't get a real certificate — `localhost` skips this entirely and self-signs one automatically.
